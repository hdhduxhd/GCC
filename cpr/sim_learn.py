...
#     parser.add_argument('--model-file', type=str, default='../logs/Domain1/20241229_050412.449571/checkpoint_185.pth.tar')
    ...
    # db_train = DL.FundusSegmentation_wsim(base_dir=args.data_dir, dataset=args.dataset, split='train/ROIs', transform=composed_transforms_train, pseudo=args.pseudo[:-4]+'_'+time+'.npz',radius=radius)
    ...
    if args.dataset == "Domain2":
        torch.save({
                    ...
                    }, './log/sim_learn_D2_'+time+'.pth.tar')
    if args.dataset == "Domain1":
        torch.save({
                    'model_state_dict': model.state_dict(),
                    }, './log/sim_learn_D1_'+time+'.pth.tar')
    if args.dataset == "Domain4":
        torch.save({
                    'model_state_dict': model.state_dict(),
                    }, './log/sim_learn_D4_'+time+'.pth.tar')
